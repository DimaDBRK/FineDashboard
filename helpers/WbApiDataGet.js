...
export const WbApiDataGet = async () => {
  ...
  console.log(countryName, "data collection finish");
  // dataToInsertDBb.push(result)
  return result
}
...
//   await getJsonData(country);
// })

const results = countryList.map(async (country)=>{
  const countryVals = await getJsonData(country);
  // console.log(indicatorVals);
  return countryVals
  
})
//return the list of lists of feature values [[val1,val2,val3...], [val1,val2,val3...], [val1,val2,val3...], ...]
const countryVals = await Promise.all(results);
// console.log("countryVals", countryVals);
countryVals.map((countryArr)=>{
  countryArr.map((row)=>{
    dataToInsertDBb.push(row);
  })
